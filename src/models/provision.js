...
const treatment = require('./control-hub/treatment')
...
async function main (userId) {
  try {
    ...
    const ruleName = await treatment.findRule(userId)
    // create new treatment rule if it doesn't exist
    if (!ruleName) {
      await treatment.create(userId)
    }
    // get the treatment ID for the user
    const treatmentId = await treatment.getId(userId)
    // get the existing treamtent ID order list
    const treatments = await treatment.listOrder(userId)
